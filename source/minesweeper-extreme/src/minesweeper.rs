...
pub struct Minesweeper {
    ...
    mine_indices: VecDeque<usize>,
    mines_per_ring: VecDeque<i32>,
...
impl Minesweeper {
    pub fn new(parent_visual: &ContainerVisual, parent_size: &Vector2, flag: String) -> winrt::Result<Self> {
        ...
        let mut result = Self {
            ...
            mine_indices: VecDeque::new(),
            mines_per_ring: VecDeque::new(),
            mine_states: Vec::new(),
            mines: Vec::new(),
            ...
            cheat_code: vec![VirtualKeyCode::X,
                 VirtualKeyCode::Y,
                 VirtualKeyCode::Z,
                 VirtualKeyCode::Z,
                 VirtualKeyCode::Y,
    ...
    pub fn on_pointer_moved(&mut self, point: &Vector2) -> winrt::Result<()> {
        ...
        // cache mine state for some extra animation performance!
        
        let current_selection = self.ui.current_selected_tile();

        if let Some(current_selection) = current_selection {
            if self.debug { 
                info!("Hovered over {:?}", (current_selection.x, current_selection.y));
            }
            ...
            if self.mine_states[index] == MineState::Empty {
                if self.sweep(current_selection.x, current_selection.y, false)? {
                    // We hit a mine! Setup and prepare an animation.
                    let hit_x = current_selection.x;
                    let hit_y = current_selection.y;

                    self.prepare_animation_on_all_mines(hit_x, hit_y)?;
                }
            }
            if self.debug { 
                info!("Animation caching finished!"); 
                self.state_info = format!("x: {:?} y: {:?} mines: {:?}", current_selection.x, current_selection.y, self.num_mines);
            }
        ...
        if shift_held {
            match virtual_keycode {
                Some(VirtualKeyCode::Return) => {
                    self.key_queue.push_front(VirtualKeyCode::Unlabeled)
                },
                _ => {},
            }
        } else if ctrl_held {
            match virtual_keycode {
                Some(VirtualKeyCode::Key1) => {
                    self.new_game(10, 10, 90)?;
                },
                Some(VirtualKeyCode::Key2) => {
                    self.new_game(12, 12, 130)?;
                },
                Some(VirtualKeyCode::Key3) => {
                    self.new_game(14, 14, 176)?;
                },
                Some(VirtualKeyCode::Key4) => {
                    self.new_game(16, 16, 230)?;
                },
                Some(VirtualKeyCode::Key5) => {
                    self.new_game(18, 18, 306)?;
                },
                Some(VirtualKeyCode::Key6) => {
                    self.new_game(20, 20, 360)?;
                },
                Some(VirtualKeyCode::Key7) => {
                    self.new_game(22, 22, 436)?;
                },
                _ => {},
            }
        } else {
            match virtual_keycode {
                ...
                Some(keycode) => { self.key_queue.push_front(keycode) },
                _ => {},
            }
        }
        self.key_queue.truncate(self.cheat_code.len());
        
        if self.key_queue.iter().rev().cloned().eq(self.cheat_code.iter().cloned()) {
        ...
        if let Some(current_selection) = current_selection {
            ...
            if self.mine_states[index] != MineState::Revealed {
                if is_right_button || is_eraser {
                    ...
                    if self.sweep(current_selection.x, current_selection.y, true)? {
                        // We hit a mine! Play the cached animation while locking any input.
                        // First, hide the selection visual and reset the selection
                        self.ui.select_tile(None)?;

                        self.play_animation_on_all_mines()?;
                        ...
                        self.game_over = true;
                        info!("Player won! The flag is {}", self.flag);
                    }
                }
            }
        }
    ...
    fn new_game(&mut self, board_width: i32, board_height: i32, mines: i32) -> winrt::Result<()> {
        ...
        info!("New game started, grid size: {:?}, {:?} mines", (board_width, board_height), mines);

        Ok(())
    }

    fn sweep(&mut self, x: i32, y: i32, reveal: bool) -> winrt::Result<bool> {
        ...
        if reveal { self.reveal(*sweeps.front().unwrap())?; }
        ...
        while !sweeps.is_empty() {
            ...
            if self.mines[index] {
                if self.debug { debug!("Hit a mine while sweeping!"); }
    ...
    fn get_surrounding_mine_count(&self, x: i32, y: i32) -> i32 {
        ...
        if self.debug { info!("Surrounding mine count is {:?}", count); }
    ...
    fn check_tile_for_mine_for_animation(
        ...
        if self.index_helper.is_in_bounds(x, y) {
            ...
            if self.mines[tile_index] {
                self.mine_indices.push_back(tile_index);
                *mines_in_ring += 1;
                if self.debug { 
                    let debug_str = format!("Mine animation cached, mines animated: {:?}, mine states: {:?}, mines: {:?}, mines per ring: {:?}, neighbor counts: {:?}", 
                                            self.mine_indices.len(),
                                            self.mine_states,
                                            self.mines,
                                            self.mines_per_ring,
                                            self.neighbor_counts);
                    self.state_info = debug_str.clone();
                    debug!("{} (detected mine at {:?})", &debug_str, (x, y)); 
                } 
            }
            *visited_tiles += 1;
        }
    ...
    fn prepare_animation_on_all_mines(&mut self, center_x: i32, center_y: i32) -> winrt::Result<()> {
        // Build a queue that contains the indices of the mines in a spiral starting from the hovered mine.
        self.mine_indices.clear();
        self.mines_per_ring.clear();
        ...
        while visited_tiles < (self.game_board_width * self.game_board_height) {
            if ring_level == 0 {
                ...
                self.mine_indices.push_back(hit_mine_index);
                self.mines_per_ring.push_back(1);
                visited_tiles += 1;
            } else {
                ...
                if current_mines_in_ring > 0 {
                    self.mines_per_ring.push_back(current_mines_in_ring);
                }
            }
            ring_level += 1;
        }
    ...
    fn play_animation_on_all_mines(&mut self) -> winrt::Result<()> {
        self.ui.play_mine_animations(self.mine_indices.clone(), self.mines_per_ring.clone())?;
