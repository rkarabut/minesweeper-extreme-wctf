...
fn run() -> winrt::Result<()> {
    ...
    window.set_title("Minesweeper EXTREME");
    ...
    let mut game = Minesweeper::new(&root, &window_size, fs::read_to_string("flag.txt")
                                    .unwrap_or("flag{FLAG_MISSING}".to_string()))?;
        ...
        *control_flow = ControlFlow::Wait;
        match event {
            ...
            Event::WindowEvent {
                ...
                if game.debug {
                    window.set_title(&format!("Minesweeper EXTREME    {}", game.state_info));
                }
            }
            Event::WindowEvent {
            ...
            Event::WindowEvent {
                ...
                if input.state == ElementState::Released {
                    game.on_key_released(input.virtual_keycode, input.modifiers.shift(), input.modifiers.ctrl())
                        .unwrap();
                }
            }
            _ => (),
